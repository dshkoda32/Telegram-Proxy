...
# Параметры
PORT=902				# Порт для прокси-сервера
DOMAIN="2gis.com"		# Домен для маскировки TLS
VERSION=""				# Версия telemt (если версия не указана то ставим последнюю релизную версию)
MAX_CONNECTIONS=4000	# Максимальное количество одновременных TCP подключений (0 = без лимита)
BUF_DIVISOR=16			# Делитель буферов - уменьшает буферы если не хватает оперативной памяти (1-16, по умолчанию без деления = 1)
...
sed -i '/^\[\[server\.listeners\]\]$/{N;N;s|^\[\[server\.listeners\]\]\nip = "::"\nport = \([0-9]\+\)|#[[server.listeners]]\n#ip = "::"\n#port = \1|;}' "$CONFIG_FILE"
